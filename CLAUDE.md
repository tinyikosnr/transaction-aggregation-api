...
| Flyway | `flyway-database-postgresql` | Schema migrations; `V1` (customers), `V2` (merchants), `V7` (audit events) exist so far on this branch |
...
- Modules are recognised by Spring Modulith's default package-based convention alone: a bare `package-info.java` is enough, no annotation required. `verify()`'s built-in encapsulation of nested (non-root) packages already enforces "no direct cross-module persistence access" and "controllers cannot depend on repositories" with zero extra configuration. Add an explicit `@ApplicationModule` annotation only where a rule genuinely cannot be expressed any other way: `shared` carries one (`type = OPEN, allowedDependencies = {}`), because "shared may not depend on business modules" has no other enforcement mechanism, and `audit` now carries one too (`allowedDependencies = "shared"`), because with another module (`transaction`) genuinely expected to depend on it, "`audit` depends on nothing else but `shared`" became a real, verifiable rule. **`audit`'s allow-list is not empty, unlike a same-looking declaration might suggest**: `shared` must be listed explicitly even though it's `type = OPEN` - `OPEN` only means other modules don't need `shared`'s permission to depend on it, it does not exempt `shared` from appearing in a module's own `allowedDependencies`. An earlier draft of this annotation used an empty `{}` by copying the pattern without checking whether the module actually referenced `shared` in code; `verify()` caught the mistake immediately (a real dependency it legitimately needed was rejected) - trust the failing test over a copied pattern. `customer` and `merchant` were both deliberately left without an annotation, for consistency: package-based detection already covers everything currently needed for them. Do not add annotations to other modules "for documentation"; the package-info Javadoc already documents intent, and an annotation should only appear when it changes verified behaviour.
- **Cross-module API exposure uses a package-level `@NamedInterface`, not per-class annotations.** Spring Modulith's default encapsulation only exposes a module's *root* package (`audit`); nested packages like `audit.application` are internal and unreachable from other modules by default, even for public types - confirmed empirically before this was relied on. `audit.application`'s `package-info.java` carries a single `@org.springframework.modulith.NamedInterface`, exposing every public type in that package (`RecordAuditEventUseCase`, `RecordAuditEventCommand`) as one deliberate API surface, rather than annotating each exported type individually. `AuditService` stays unreachable regardless, since it's package-private. This is the first module on this branch to actually need cross-module exposure (`customer` and `merchant` haven't needed a caller from another module yet); follow the same package-level pattern, not per-class, when the next module needs it.
...
**Current status:** `project-structure`, `shared`, `customer`, and `merchant` are complete and merged. `categorisation` and `audit` were built as sibling branches off the same post-merchant `main` commit, since neither depends on the other (`transaction → categorisation` and `transaction → audit` are independent edges) - only `transaction` depends on both. Whichever of the two merges second will hit a `CLAUDE.md` conflict in this file (both branches document their own module in the same sections) and should reconcile it by keeping both modules' content, not discarding either side. `feature/transaction` was started once before either of these existed, then paused and fully reverted: `transactions.category_id` is a `NOT NULL` foreign key to `transaction_categories`, a table `categorisation` didn't exist to own yet at that point, so `transaction` could not have been built and tested against a real categorisation decision. Splitting `transaction` into a "buildable now" and "blocked" tier was considered and explicitly rejected in favour of finishing `categorisation` and `audit` first, then resuming `transaction` as a single, un-split branch once both are merged into `main`. Do not re-attempt a partial/tiered `transaction` implementation. When `transaction` resumes, also revisit its persisted `TransactionStatus` values against the now-implemented categorisation and audit workflows, to confirm no documented intermediate state is being lost.
