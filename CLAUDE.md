...
│   │                          CreateTransactionsBulkUseCase, BulkCreateTransactionsCommand,
...
│   │                          (bulk additions, feature/transaction-bulk: FR-02/UC-02/TDS 29 -
│   │                          CreateTransactionsBulkUseCase calls CreateTransactionUseCase once per item
│   │                          rather than duplicating its workflow; the complete bulk outcome, including
│   │                          per-item structural validation and total/successful/failed counting, is
│   │                          owned entirely here, never split with the api layer - see the dedicated
│   │                          note under Implementation Rules below)
...
**`feature/transaction-bulk` (post-MVP, after `feature/transaction-query`) closed FR-02/UC-02/SAD 32.5/35.2/39.8/TDS 29: `POST /api/v1/transactions/bulk`, `TRANSACTION_WRITE`-protected, up to 500 items per request, partial success.** Category administration, audit querying, OpenAPI, observability, rate limiting, and any parallel/async processing remained explicitly out of scope. Three design points were deliberately resolved in a specific way, each worth restating so a future reader doesn't reintroduce the mistake:
- **The complete bulk outcome is owned entirely by `transaction.application`, never split with the api layer.** `CreateTransactionsBulkUseCase.create(BulkCreateTransactionsCommand)` accepts the *whole* submitted batch (as `BulkTransactionItemInput` - raw, unvalidated, nullable-field records, deliberately distinct from `CreateTransactionCommand`) and returns the *whole* result (`BulkTransactionResult`, with `total`/`successful`/`failed` already computed, in submission order). The api layer (`TransactionController.createBulk`, `TransactionApiMapper.toBulkCommand`/`toBulkResponse`) does pure JSON binding and a 1:1 field-for-field translation only - it does not filter items, does not merge two separate result lists, and does not compute counts itself. This was a deliberate correction of an earlier draft that had the api layer pre-validating structurally-bad items and merging its own failures with the application layer's results by index; that design was rejected specifically because it left `transaction.application`'s own notion of `total`/`successful`/`failed` unable to see items the api layer had already rejected, an incomplete/misleading count. `BulkCreateTransactionsCommand.transactions` in the API request DTO is deliberately **not** `@Valid`-annotated for exactly the same reason `@Valid` isn't used per-item anywhere in this bulk path: cascading Bean Validation would fail the *entire* request the moment any single item violates a field constraint, contradicting SAD 32.5/39.8's partial-success requirement.
- **Per-item structural validation happens inside `BulkCreateTransactionsService`, not via `CreateTransactionCommand`'s own `Objects.requireNonNull` compact constructor.** A missing required field on a `BulkTransactionItemInput` is checked explicitly and translated into a `TransactionValidationException` - the same exception type every other per-item validation failure in this class already produces - before `CreateTransactionCommand` is ever constructed. Relying on `CreateTransactionCommand`'s constructor to reject the null (via `NullPointerException`) was considered and rejected: an NPE is reserved for programming errors elsewhere in this codebase, not expected per-item input handling, and it isn't one of the exception types `BulkCreateTransactionsService` catches (see the next point) - letting it happen would have meant either adding it to the catch list (blurring "expected" and "unexpected" failures) or having it silently abort the whole batch.
- **`BulkCreateTransactionsService` catches exactly four known exception types per item - `DuplicateTransactionException`, `CustomerNotFoundException`, `TransactionSourceNotFoundException`, `TransactionValidationException` - and nothing broader.** There is deliberately no catch-all `RuntimeException` handler. SAD 32.5/39.8 describe an "invalid item" - a business/validation-shaped failure of that item's own data - never a systemic condition, and 39.8's "must not return a successful overall status that hides item-level failures" cuts against silently folding an infrastructure failure into an ordinary-looking per-item result. An unexpected exception (a `DataAccessException` from a database outage, or any other unmapped `RuntimeException`) is allowed to propagate out of `create`, aborting the whole bulk HTTP request via the existing, unmodified whole-request `500`/`503` handling in `GlobalExceptionHandler` - items already processed before that point stay committed (each ran in its own already-completed transaction; see the transaction-boundary point below), but the client receives no 207 body for the aborted remainder and must reconcile via `GET /api/v1/transactions`.
