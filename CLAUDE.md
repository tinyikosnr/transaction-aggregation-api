...
| Flyway | `flyway-database-postgresql` | Schema migrations; `V1` (customers), `V2` (merchants), `V3`/`V4` (categorisation tables), `V5`/`V6` (categorisation seed data) exist so far |
...
**A module's shape follows its own documented capabilities, not the previous module's shape.** `customer` has three inbound ports (lookup, exists, register) and two exceptions because three separate capabilities and two distinct failure scenarios are documented for it. `merchant` has one inbound port (`MerchantResolutionPort`, a fused find-or-create operation) and one exception (`DuplicateMerchantException`, used internally for recovery, not a caller-facing "not found"/"conflict" pair) because that's what its own documentation calls for. `categorisation` has a single inbound port (`CategoriseTransactionUseCase`) and no application-level exception at all: the only "nothing matched" case is handled internally by falling back to the seeded fallback category, not by throwing to a caller (see `CategorisationService`). When starting a new module, work out what *that module* needs from the SAD/TDS before assuming it should look like the last one.
...
- Modules are recognised by Spring Modulith's default package-based convention alone: a bare `package-info.java` is enough, no annotation required. `verify()`'s built-in encapsulation of nested (non-root) packages already enforces "no direct cross-module persistence access" and "controllers cannot depend on repositories" with zero extra configuration. Add an explicit `@ApplicationModule` annotation only where a rule genuinely cannot be expressed any other way: `shared` carries one (`type = OPEN, allowedDependencies = {}`), because "shared may not depend on business modules" has no other enforcement mechanism, and `categorisation` now carries one too (`allowedDependencies = {}`), because with another module (`transaction`) genuinely expected to depend on it, "`categorisation` depends on nothing else" became a real, verifiable rule rather than an incidental fact. `customer` and `merchant` were both deliberately left without one, for consistency: package-based detection already covers everything currently needed for them. Do not add annotations to other modules "for documentation"; the package-info Javadoc already documents intent, and an annotation should only appear when it changes verified behaviour.
- **Cross-module API exposure uses a package-level `@NamedInterface`, not per-class annotations.** Spring Modulith's default encapsulation only exposes a module's *root* package (`categorisation`); nested packages like `categorisation.application` are internal and unreachable from other modules by default, even for public types, confirmed empirically before this was relied on. `categorisation.application`'s `package-info.java` carries a single `@org.springframework.modulith.NamedInterface`, exposing every public type in that package (`CategoriseTransactionUseCase`, `CategorisationInput`, `CategorisationDecision`) as one deliberate API surface, rather than annotating each exported type individually. `CategorisationService` stays unreachable regardless, since it's package-private. This is the first module to actually need cross-module exposure (`customer` and `merchant` haven't needed a caller from another module yet); follow the same package-level pattern, not per-class, when the next module needs it.
...
**Current status:** `project-structure`, `shared`, `customer`, `merchant`, and `categorisation` are complete. `feature/transaction` was started once, then paused and fully reverted: `transactions.category_id` is a `NOT NULL` foreign key to `transaction_categories`, a table `categorisation` didn't exist to own yet at that point, so `transaction` could not have been built and tested against a real categorisation decision. Splitting `transaction` into a "buildable now" and "blocked" tier was considered and explicitly rejected in favour of finishing `categorisation` (and `audit`, if the workflow still requires it) first, then resuming `transaction` as a single, un-split branch. Do not re-attempt a partial/tiered `transaction` implementation; treat `categorisation` (and `audit`, if applicable) as a hard prerequisite, not an optional nice-to-have. When `transaction` resumes, also revisit its persisted `TransactionStatus` values against the now-implemented categorisation workflow, to confirm no documented intermediate state is being lost.
